...
import {
  OPEN_ACCOUNT_CURRENCY, INTERNATIONAL_TRANSFER_TEXT, OPEN_SAVINGS_ACCOUNT_TEXT,
  CURRENCY_LIST_OBJ, MIN_AMOUNT_FOR_OPENING_SAVINGS_ACCOUNT,
...
export class SavingsAccountComponent implements OnInit {
  ...
  calculateMinimumAmount(): void {
    switch (this.targetAccountCurrency) {
      case CURRENCY_LIST_OBJ.USD:
        this.minimumAmountForOpeningAccount = MIN_AMOUNT_FOR_OPENING_SAVINGS_ACCOUNT.USD;
        break;
      case CURRENCY_LIST_OBJ.AED:
        this.minimumAmountForOpeningAccount = MIN_AMOUNT_FOR_OPENING_SAVINGS_ACCOUNT.AED;
        break;
      case CURRENCY_LIST_OBJ.GBP:
        this.minimumAmountForOpeningAccount = MIN_AMOUNT_FOR_OPENING_SAVINGS_ACCOUNT.GBP;
        break;
    }
  }
  ...
  setSummaryDetails(): void {
    this.openAccountService.savingsAccountSummaryDetails.depositAmount =
      this.openAccountFormGroup.get(OPEN_SAVINGS_ACCOUNT_TEXT.DEPOSIT_AMOUNT).value ?
        this.sharedService.removeCommaFromString(this.openAccountFormGroup.get(OPEN_SAVINGS_ACCOUNT_TEXT.DEPOSIT_AMOUNT).value) :
        this.openAccountFormGroup.get(OPEN_SAVINGS_ACCOUNT_TEXT.DEPOSIT_AMOUNT).value;
    this.openAccountService.savingsAccountSummaryDetails.mobileNumber =
      this.openAccountFormGroup.get(OPEN_SAVINGS_ACCOUNT_TEXT.MOBILE_NUMBER).value;
    this.openAccountService.savingsAccountSummaryDetails.emailId =
      this.openAccountFormGroup.get(OPEN_SAVINGS_ACCOUNT_TEXT.EMAIL_ID).value;
    this.openAccountService.savingsAccountSummaryDetails.payFromAccount =
      this.openAccountFormGroup.get(OPEN_SAVINGS_ACCOUNT_TEXT.TRANSFER_FROM_ACCOUNT).value;
  }
  ...
  getExchangeRate(): void {
    if (this.openAccountService.savingsAccountSummaryDetails && this.openAccountService.savingsAccountSummaryDetails.targetAccountCurency
      ...
      exchangeRatepayLoad.transferAmount = OPEN_SAVINGS_ACCOUNT_TEXT.EXCHANGE_TRANSFER_AMOUNT;
  ...
  validateAccountBalance(): void {
    const selectAccountOption = this.openAccountFormGroup.get(OPEN_SAVINGS_ACCOUNT_TEXT.TRANSFER_FROM_ACCOUNT);
    ...
    } else if (this.openAccountFormGroup.get(OPEN_SAVINGS_ACCOUNT_TEXT.DEPOSIT_AMOUNT).value) {
  ...
  calculateDebitAmount(): void {
    let amount = this.openAccountFormGroup.get(OPEN_SAVINGS_ACCOUNT_TEXT.DEPOSIT_AMOUNT).value;
  ...
  validateAmount(): void {
    ...
    if (amount) {
      if (amount < this.minimumAmountForOpeningAccount) {
        this.openAccountFormGroup.get(OPEN_SAVINGS_ACCOUNT_TEXT.DEPOSIT_AMOUNT).setErrors(
          {
  ...
  showAccountFormComponent(isBackBtnClicked: boolean): void {
    isBackBtnClicked ? this.cancelOpeningAccountProcess() : (this.showSummaryScreen = false,
      this.preSelectedCurrency = this.openAccountService.savingsAccountSummaryDetails.targetAccountCurency);
  }
